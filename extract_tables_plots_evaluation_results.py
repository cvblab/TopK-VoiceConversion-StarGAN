...
def plot_grouped_comparison(df_dict, keys, bar_width=1, group_spacing=0.7):
    ...
    for i, key in enumerate(keys):
        key_df = df_dict[key]
        mean_values_key = key_df['mean']/3
        std_values_key = key_df['std']/3
        ...
        # Plot the mean values with error bars representing the std
        #plt.errorbar(group_positions, mean_values_key, yerr=std_values_key, fmt='none', capsize=0.5, capthick=0.1, color='black')
...
def main(config):
    ...
    #
    df = pd.read_csv(os.path.join(config.experiment_ev_results_folder, ev_results_csvs[11]))
    df["MCD Mean"] = df["MCD Mean"]
    df["MCD STD"] = df["MCD STD"]
    ...
    for ev_results_csv in ev_results_csvs:
        ...
        df["MCD Mean"] = df["MCD Mean"] / 2.5
        df["MCD STD"] = df["MCD STD"] / 2.5
        df_grouped_by_n_iter = compute_grouped_mean_std(df)
    ...
    for i in range(1, len(keys), plot_every):
        print(i)
        plot_name =str(i) + ".png"
        #plot_grouped_comparison_lines(dfs_dict, keys=[keys[9]], savefig=True, savefig_path=os.path.join(config.figures_savedir, plot_name))
